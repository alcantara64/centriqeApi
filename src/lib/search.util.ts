...
export type Operator = '='
  | "<>" | "!=" /**2021-02-18 - Ritesh JIRA#CA-128 https://centriqe-dev.atlassian.net/browse/CA-128 */
  | ">"
  | "<"
  | ">="
  | "<="
  ...
  | "contains" | "Contains"
  | "Does not contain" | "does not contain" /**2021-02-18 - Ritesh JIRA#CA-128 https://centriqe-dev.atlassian.net/browse/CA-128 */
  | "days before" | "Days Before"
  | "days after" | "Days After"
  | "days after (no year)"
  | "days before (no year)"
  | "is populated" | "Is populated"  /**2021-02-18 - Ritesh JIRA#CA-128 https://centriqe-dev.atlassian.net/browse/CA-128 */
  | "is not populated" | "Is not populated"; /**2021-02-18 - Ritesh JIRA#CA-128 https://centriqe-dev.atlassian.net/browse/CA-128 */
 ...
 * @param criteria
 */
function convertCriteriaToMongooseQueryAndAttachOrgLimiter(criteria: Criteria, orgLimiter: QueryLimiter, modelStatus?: ModelStatus[]): any {
  ...
  query = attachModelStatusLimiter(query, modelStatus);

  return query;
}


export function attachModelStatusLimiter(query: any, modelStatus?: ModelStatus[]) {
  if (modelStatus) {
    query = {
      $and: [
        query,
        { status: { $in: modelStatus } }
      ]
    }
  }
...
export function convertCriterionToCondition(criterion: Criterion): any {
  ...
  switch (operator) {
    ...
    case "Contains":
    case "contains":
      /** 2021-02-09 - Frank - Updated to mongoose regex format -- this way it serializes and de-serializes from/ to json format*/
      condition[criterion.attributeName] = { $regex: `${criterion.values[0]}`, $options: 'i' };
      break;

    /**2021-02-18 - Ritesh JIRA#CA-128 https://centriqe-dev.atlassian.net/browse/CA-128 */
    case "Does not contain":
    case "does not contain":
      condition[criterion.attributeName] = { $not: { $regex: `${criterion.values[0]}`, $options: 'i' } };
      break;
    case "Is populated":
    case "is populated":
      const existIsPopulated: any = {};
      const nullIsPopulated: any = {}
      existIsPopulated[`${criterion.attributeName}`] = { $exists: true };
      nullIsPopulated[`${criterion.attributeName}`] = { $ne: null };
      condition = { $and: [existIsPopulated, nullIsPopulated] }
      break;
    case "Is not populated":
    case "is not populated":
      const existNotPopulated: any = {};
      const nullNotPopulated: any = {}
      existNotPopulated[`${criterion.attributeName}`] = { $exists: false };
      nullNotPopulated[`${criterion.attributeName}`] = { $eq: null };
      condition = { $or: [existNotPopulated, nullNotPopulated] }
      break;
    /**2021-02-18 - END of -- Ritesh JIRA#CA-128 https://centriqe-dev.atlassian.net/browse/CA-128 */
    case "Days After":
    case "days after":
    case "Days Before":
    case "days before": {
      condition = buildDaysRangeCondition("date", criterion, operator);
      break;
    }

    case "days after (no year)":
    case "days before (no year)": {
      condition = buildDaysRangeCondition("string", criterion, operator);
      break;
    }
 ...
 */
export function buildDaysRangeCondition(attributeType: "string" | "date", criterion: Criterion, operator: Operator): any {
  const days = criterion.values[0];
  let date = DateTime.local().setZone("UTC", { keepLocalTime: true }).set({ minute: 0, second: 0, millisecond: 0 });
  if (operator === "Days After" || operator === "days after" || operator === "days after (no year)") {
    date = date.plus({days: days});
  }
  else if (operator === "Days Before" || operator === "days before" || operator === "days before (no year)") {
  ...
  let queryDate: string | Date

  if (attributeType === "string") {
    queryDate = generateDateNoYearString(date.toJSDate());
  } else {
    queryDate = date.toJSDate()
  }

  const dateCondition: any = {};
  dateCondition[`${criterion.attributeName}`] = { $eq: queryDate };
  // const endDateCondition: any = {}
  // endDateCondition[`${criterion.attributeName}`] = { $lte: end };
